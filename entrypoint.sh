...
if [[ $? != 0 ]]; then
    ...
    echo '========================================================='
    echo $tasks
    echo "dbt build --target dev --profiles-dir ./trino_profile --project-dir ./zettablock --select ${tasks[@]} --vars '{\"external_s3_location\":\"s3://my-897033522173-us-east-1-spark/demo/$(openssl rand -hex 8)/\", \"TRINO_USER\":\"'\"$TRINO_USER\"'\", \"TRINO_PASSWORD\":\"'\"$TRINO_PASSWORD\"'\", \"TRINO_HOST\":\"'\"$TRINO_HOST\"'\"}' --full-refresh"
    echo '========================================================='
    echo "dbt build --target dev --profiles-dir ./trino_profile --project-dir ./zettablock --select ${tasks[@]} --vars '{\"external_s3_location\":\"s3://my-897033522173-us-east-1-spark/demo/$(openssl rand -hex 8)/\", \"TRINO_USER\":\"'\"$TRINO_USER\"'\", \"TRINO_PASSWORD\":\"'\"$TRINO_PASSWORD\"'\", \"TRINO_HOST\":\"'\"$TRINO_HOST\"'\"}' --full-refresh"| bash
    if [ $? -ne 0 ]
        then
            echo "exception."
            exit 255
